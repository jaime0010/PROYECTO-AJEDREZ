...
void Caballo::dibuja()
{
    ...
    if (color == 0)
    {
        ...
        glEnable(GL_TEXTURE_2D);         glBindTexture(GL_TEXTURE_2D, ETSIDI::getTexture("caballo-negrop.png").id);
    ...
    if (color == 255)
    {
        ...
        glEnable(GL_TEXTURE_2D);         glBindTexture(GL_TEXTURE_2D, ETSIDI::getTexture("caballo-blancop.png").id);
        glDisable(GL_LIGHTING);
        glBegin(GL_POLYGON);
...
bool Caballo::validar_movimiento(Vector2D p_fin, ListaPiezas* lista)
{
    ...
    dist.x = abs(p_fin.x - posicion.x);
    dist.y = abs(p_fin.y - posicion.y);
