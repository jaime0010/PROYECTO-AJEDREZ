...
void Peon::dibuja()
{
    ...
    if (color == 0) //Peones Blancos
    {
        ...
        glEnable(GL_TEXTURE_2D);         glBindTexture(GL_TEXTURE_2D, ETSIDI::getTexture("peon-negrop.png").id);
    ...
    else if (color == 255) //Peones Negros
    {
        ...
        glEnable(GL_TEXTURE_2D);         glBindTexture(GL_TEXTURE_2D, ETSIDI::getTexture("peon-blancop.png").id);
        glDisable(GL_LIGHTING);
        glBegin(GL_POLYGON);
...
//Método INCORRECTO
//bool Peon::validar_movimiento(Vector2D ini, Vector2D fin, Pieza* p)
//{
//    return false;
//}
...
bool Peon::validar_movimiento(Vector2D p_fin, ListaPiezas* lista)
{
    ...
    //int distx = p_fin.x - posicion.x;
    //int disty = p_fin.y - posicion.y;
    //Sobrecargar - y jugar con dist (POO encapsulamiento)
